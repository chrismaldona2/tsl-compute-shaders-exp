...
const permute_0 = Fn(([x_immutable]: [Node]) => {
...
const permute_1 = Fn(([x_immutable]: [Node]) => {
...
export const permute = overloadingFn([permute_0, permute_1] as any);

const taylorInvSqrt_0 = Fn(([r_immutable]: [Node]) => {
...
const taylorInvSqrt_1 = Fn(([r_immutable]: [Node]) => {
...
export const taylorInvSqrt = overloadingFn([
  taylorInvSqrt_0,
  taylorInvSqrt_1,
] as any);

export const grad4 = Fn(([j_immutable, ip_immutable]: [Node, Node]) => {
...
export const simplexNoise4d = Fn(([v_immutable]: [Node]) => {
  ...
  p2.mulAssign(norm.z);
  p3.mulAssign(norm.w);
  p4.mulAssign(taylorInvSqrt(dot(p4, p4)) as unknown as Node);
